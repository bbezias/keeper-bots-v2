...
const MAX_TX_PACK_SIZE = 1232; //1232;
...
export class FillerBot implements Bot {
  ...
  private async updatePrice() {
    for (const perp of this.driftClient.getPerpMarketAccounts().values()) {
  ...
  public async startIntervalLoop(_intervalMs: number) {
    ...
    const intervalRefreshPrices = setInterval(this.updatePrice.bind(this), 60000);
    this.intervalIds.push(intervalRefreshPrices);
